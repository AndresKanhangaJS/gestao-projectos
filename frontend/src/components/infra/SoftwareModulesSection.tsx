...
import { createSoftwareModule, deleteSoftwareModule, listSoftwareModules, updateSoftwareModule } from '@/api/infra'
...
function ModuleFormDialog({
  ...
  const mutation = useMutation({
    mutationFn: (values: FormValues) => {
      ...
      return module ? updateSoftwareModule(module.id, payload) : createSoftwareModule(productId, payload)
    },
    onSuccess: () => {
      ...
      <DialogContent>
        ...
        <form className="flex flex-col gap-4" noValidate onSubmit={handleSubmit((v) => mutation.mutate(v))}>
          ...
          <FormField id="mod-description" label="Descrição" error={errors.description}>
            <Textarea {...fieldA11y('mod-description', errors.description)} {...register('description')} />
          </FormField>
...
export function SoftwareModulesSection({ productId, canWrite }: { productId: number; canWrite: boolean }) {
  const queryClient = useQueryClient()
  const [edit, setEdit] = useState<EditState>({ open: false })
  const [toDelete, setToDelete] = useState<SoftwareModule | null>(null)
  const query = useQuery({ queryKey: softwareModulesKey(productId), queryFn: () => listSoftwareModules(productId) })
    ...
    <Card>
      <CardHeader className="flex-row items-start justify-between gap-2">
        <div className="flex flex-col gap-1.5">
          ...
          <CardDescription>Módulos que podem ser activados por cliente.</CardDescription>
        </div>
        {canWrite && (
      ...
      <CardContent>
        {query.isLoading ? (
          ...
          <EmptyState title="Este produto ainda não tem módulos" />
        ) : (
