...
import { DEPLOYMENT_COMPONENT_LABEL, DEPLOYMENT_STATUS_LABEL, MACHINE_ENVIRONMENT_LABEL } from '@/lib/labels'
...
export function DeploymentsTable({
  ...
  return (
    <>
            ...
            return (
              <TableRow key={d.id}>
                ...
                <TableCell>
                  {instance ? (
                    <>
                      <Link to={`/infra/clients/${instance.client_id}`} className="text-primary hover:underline">
                ...
                <TableCell className="text-xs text-muted-foreground">{formatDateTime(d.last_checked_at)}</TableCell>
                {canWrite && (
      ...
      {editing && <DeploymentFormDialog key={editing.id} deployment={editing} onClose={() => setEditing(null)} />}
      <DeleteConfirmDialog
        key={toDelete?.id ?? 'none'}
        item={toDelete}
        onClose={() => setToDelete(null)}
        title="Apagar deployment?"
        description={toDelete ? `“${deploymentLabel(toDelete)}” será removido, com as suas credenciais e backups.` : undefined}
