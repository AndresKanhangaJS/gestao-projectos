...
export function WorkspaceFormDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
      ...
      <DialogContent>
        ...
        <form className="flex flex-col gap-4" noValidate onSubmit={handleSubmit((values) => mutation.mutate(values))}>
          ...
          <FormField id="ws-description" label="Descrição" error={errors.description}>
            <Textarea {...fieldA11y('ws-description', errors.description)} {...register('description')} />
          </FormField>
