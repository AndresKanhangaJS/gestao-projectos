...
const schema = z.object({
  type: z.enum(['blocks', 'blocked_by', 'relates_to', 'duplicates'], { error: 'Seleccione o tipo de relação.' }),
...
export function RelationsSection({
  ...
  const relationsQuery = useQuery({ queryKey: taskRelationsKey(taskId), queryFn: () => listRelations(taskId) })
  const tasksQuery = useQuery({ queryKey: projectTasksKey(projectId), queryFn: () => listTasks(projectId) })
  ...
  const {
    ...
    setError,
    formState: { errors },
  } = useForm<FormValues>({ resolver: zodResolver(schema), defaultValues: { type: 'relates_to', related_task_id: '' } })
  ...
  const createMutation = useMutation({
    mutationFn: (values: FormValues) =>
      createRelation(taskId, { type: values.type, related_task_id: Number(values.related_task_id) }),
    onSuccess: () => {
  ...
  const results = term
    ? (tasksQuery.data ?? [])
        .filter((t) => t.id !== taskId && (t.title.toLowerCase().includes(term) || String(t.id) === term))
        .slice(0, MAX_RESULTS)
    : []
  ...
  return (
    <section aria-labelledby={`task-${taskId}-relations`}>
      <h3 id={`task-${taskId}-relations`} className="mb-2 text-sm font-semibold">
        Relações
      </h3>
      {relationsQuery.isLoading ? (
        ...
        <ul className="mb-2 flex flex-col divide-y divide-border rounded-md border border-border">
          {relations.map((relation) => {
            const title =
              relation.related_task?.title ?? tasksById.get(relation.related_task_id)?.title ?? `#${relation.related_task_id}`
            return (
      ...
      >
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-[10rem_1fr]">
          <FormField id={`task-${taskId}-relation-type`} label="Tipo" error={errors.type}>
            <SelectField
              ...
              invalid={!!errors.type}
              options={optionKeys(TASK_RELATION_LABEL).map((k) => ({ value: k, label: TASK_RELATION_LABEL[k] }))}
            />
          </FormField>
          <FormField id={searchId} label="Pesquisar tarefa do projecto" error={errors.related_task_id}>
            <Input
              id={searchId}
              value={search}
        ...
        {results.length > 0 && !selectedTask && (
          <ul className="flex flex-col rounded-md border border-border" aria-label="Resultados da pesquisa">
            {results.map((t) => (
              <li key={t.id}>
                <button
                  type="button"
