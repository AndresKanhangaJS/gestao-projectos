...
import { TOO_MANY_REQUESTS_MESSAGE, httpStatus, serverMessage } from '@/lib/errors'
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      if (status === 419) {
        ...
        setServerError(serverMessage(error) ?? 'A sessão expirou. Recarregue a página e tente novamente.')
      } else if (status === 429) {
        setServerError(TOO_MANY_REQUESTS_MESSAGE)
      } else {
        setServerError('Credenciais inválidas. Verifique o email e a palavra-passe.')
      }
    }
  }
